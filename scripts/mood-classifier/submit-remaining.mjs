/**
 * Submit remaining movies (batches 2-4) with proper JSON schema enforcement.
 * Polls each batch to completion and downloads results.
 *
 * Usage: node scripts/mood-classifier/submit-remaining.mjs
 */
...
function buildUserPrompt(movie) {
  ...
  parts.push(`\nScore this movie on all dimensions. Use the full range of each scale. Respond with JSON only.`);
  return parts.join("\n");
}
...
async function main() {
  ...
  for (let bIdx = 0; bIdx < batches.length; bIdx++) {
    ...
    const requests = batchMovies.map((movie) => ({
      ...
      params: {
        ...
        messages: [
          {
            role: "user",
            content: buildUserPrompt(movie) + "\n\nIMPORTANT: Respond ONLY with a JSON object matching the required schema. No markdown, no prose, no explanation outside the JSON.",
          },
        ],
      },
    }));
    ...
    appendFileSync(BATCH_LOG, JSON.stringify({
      ...
      movieCount: batchMovies.length, hasJsonSchema: true,
    }) + "\n");
    ...
    for await (const result of results) {
      ...
      try {
        ...
        moodScore = JSON.parse(text);
      } catch {
        // Try extracting JSON from markdown code block
        const jsonMatch = text.match(/```(?:json)?\s*(\{[\s\S]*?\})\s*```/);
        if (jsonMatch) {
          try { moodScore = JSON.parse(jsonMatch[1]); } catch { failed++; continue; }
        } else {
          // Try finding raw JSON object
          const rawJson = text.match(/\{[\s\S]*\}/);
          if (rawJson) {
            try { moodScore = JSON.parse(rawJson[0]); } catch { failed++; continue; }
          } else {
            failed++;
            continue;
          }
  ...
  console.log(`=== COMPLETE: ${finalCount}/${allMovies.length} classified ===`);
}
